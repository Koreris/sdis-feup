...
public class ControlChannelPacketHandler implements Runnable{
	...
	public void run() 
	{
		...
		case "GETCHUNK":
			...
			break;
		case "REMOVED":
			handleRemoved(headerComponents);
			break;
		default:
			break;
		}
	...
	private void handleRemoved(String[] headerComponents) {
		...
		if(Files.exists(chunkpath)) {
			int curr_rep_degree=main_server.records_store.get(headerComponents[3]+":"+headerComponents[4]+":"+chunk_size)-1;
			main_server.records_store.put(headerComponents[3]+":"+headerComponents[4]+":"+chunk_size,curr_rep_degree);
			if(curr_rep_degree<main_server.records_store.get(headerComponents[3]+":"+headerComponents[4])) {
				...
				if(curr_rep_degree<main_server.records_store.get(headerComponents[3]+":"+headerComponents[4])) {
					try {
						//send putchunk
						byte[] header=CreateMessages.createHeader("PUTCHUNK", main_server.protocol_version, main_server.id, headerComponents[3], Integer.parseInt(headerComponents[4]), main_server.records_store.get(headerComponents[3]+":"+headerComponents[4]));
						...
						DatagramPacket packet = new DatagramPacket(combined, 0, combined.length,InetAddress.getByName(main_server.data_address),main_server.data_port);
						socket.send(packet);
					}
